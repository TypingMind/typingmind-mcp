...
const getSafePath = (requestedPath) => {
  ...
  // Normalize the path and remove any ".." traversal attempts
  ...
  return path.resolve(STORAGE_BASE_DIR, normalized);
};
  ...
  // 4. LIST: GET /list - Recursive, root-relative paths (plugin compatible)
  app.get('/list', auth, (req, res) => {
    const requestedPath = req.query.path || req.query.directory || STORAGE_BASE_DIR;
    const targetPath = path.isAbsolute(requestedPath)
      ? path.normalize(requestedPath)
      : path.resolve('/', requestedPath);   // force relative to root
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            // ✅ If target is root, don't recurse into system directories
            if (targetPath === '/' && ['/proc','/sys','/dev','/run','/boot','/media','/mnt'].includes(fullPath)) {
              continue;
            }
            getFilesRecursive(fullPath, fileList);
          } else {
            // ✅ Always return path relative to root
...
let files;
if (targetPath === '/') {
  // Only scan important directories to avoid huge OS traversal
  const rootsToScan = ['/app', '/outputs', '/tmp'];
  files = [];
  ...
  files = getFilesRecursive(targetPath);
}
