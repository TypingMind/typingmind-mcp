...
async function start(authToken) {
  ...
  const SHARED_STORAGE_DIR = '/outputs';

  // 1. DOWNLOAD: GET /files/:filename?token=...
  // Uses query param token so it works directly in browser links
  app.get('/files/:filename', (req, res) => {
    ...
    const safePath = path.join(SHARED_STORAGE_DIR, path.basename(filename));
  ...
  // 2. DELETE: DELETE /files/:filename
  // Uses standard Bearer Auth
  ...
  // Uses Raw Stream for efficiency. Requires 'X-Filename' header and Bearer Auth.
  app.post('/upload', auth, (req, res) => {
    ...
    // Stream the request body directly to the GCS FUSE mount
    req.pipe(fileStream);
