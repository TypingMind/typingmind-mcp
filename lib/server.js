...
const getSafePath = (requestedPath) => {
  ...
  // 4. LIST: GET /list - Shows hierarchical structure with full paths
  app.get('/list', auth, (req, res) => {
    const buildTree = (dirPath) => {
      ...
      try {
        ...
        // Permission denied or other error - just return empty
    ...
    try {
      // Support ?path=/outputs to list specific directory
      const requestedPath = req.query.path || STORAGE_BASE_DIR;
